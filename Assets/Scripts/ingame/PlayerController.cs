...
public class PlayerController : NetworkBehaviour
{
    ...
    public override void FixedUpdateNetwork()
    {
        ...
        if (initCount == 1)
        {
            ...
            myTeam = myInfo.GetComponent<PlayerInfo>().team;
    ...
    private void StartDrifting(NRunner.NetworkInputData input)
    {
        ...
        for (int i = 2; i < 4; i++)
        {
            ...
            wheels[i].steerAngle = Input.GetAxis("Horizontal") * rot2;
        }
        // JoyStick
        for (int i = 2; i < 4; i++)
        {
    ...
    private void StopDrifting()
    {
        for (int i = 2; i < 4; i++)
        {
            ...
            wheels[i].steerAngle = Input.GetAxis("Horizontal") * 0;
        }
        // JoyStick
        for (int i = 2; i < 4; i++)
        {
        ...
        if (input.IsDownThisFrame(NRunner.NetworkInputData.UseItemOne))
        {
            if (itemList.Count > 0)
            {
                switch (itemList[0])
                {
                    ...
                    case 3: audio.PlayBooster(); Booster.instance.OnClick(); break;
