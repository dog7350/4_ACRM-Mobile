...
public class RoomPVP : NetworkBehaviour
{
    ...
    void U_SelectCar(string carname, GameObject obj)
    {
        ...
        HpImg.fillAmount = (car.hp * 0.01f) / 1f;
        DefImg.fillAmount = (car.def * 0.01f) / 0.85f;
        SpdImg.fillAmount = (car.speed * 0.01f) / 0.015f;
    }
    void U_SelectGun(string gunname, GameObject obj)
    {
        ...
        AtkImg.fillAmount = (gun.atk * 0.01f) / 0.95f;
        AmmoImg.fillAmount = (gun.ammo * 0.01f) / 2f;
        RpmImg.fillAmount = (gun.rpm * 0.01f) / 0.015f;
    }
